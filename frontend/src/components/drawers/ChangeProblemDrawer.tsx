...
const ChangeProblemDialog = () => {
  ...
  const currentCreator = useProblemStore((s) => s.creator);
  ...
  const handleProblemSelect = (selectedProblem: SelectedProblem) => {
    ...
    const isProblemChanged =
      ...
      selectedProblem.creator.id !== originProblem?.creator.id ||
  ...
  return (
    <ProblemDrawer
      ...
      triggerClassName="h-full text-lg cursor-pointer font-ownglyph text-pc-title-sm font-bold hover:bg-point-200/50 transition-colors duration-200"
      dialogTitle="사건 변경"
    />
  );
};
